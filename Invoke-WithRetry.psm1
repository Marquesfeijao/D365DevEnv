    ...
    for ($attempt = 1; $attempt -le $MaxAttempts; $attempt++) {
        try {
            ...
            if ($attempt -ge $MaxAttempts) {
                Write-Host "$OperationName failed after $MaxAttempts attempts`: $($_.Exception.Message)"
                throw
            }

            Write-Host "$OperationName failed (attempt $attempt of $MaxAttempts): $($_.Exception.Message). Retrying in $DelaySeconds seconds..."
